...
### 5.6 Three things the shell deliberately leaves to the deployment

Phase 9 built the API and the officer console. Three things they need are
decisions rather than code, and each is currently absent rather than guessed:

- **How staff authenticate.** The console trusts the officer identifier typed
  into it, and the API is unauthenticated. `deploy/compose.yaml` therefore binds
  the port to localhost. Whatever answers this — a card, a directory, a shared
  terminal that one supervisor unlocks — is a decision about how the post
  actually works, and inventing one would produce attributable-looking records
  that attribute nothing.
- **Where signed checkpoints are published.** The system signs them on request.
  Until they are carried somewhere outside this box, the log proves nothing to
  anyone who does not already trust the operator; see
  [ADR 0003](adr/0003-transparency-log-over-fabric.md). A weekly copy to a
  second organisation is enough. Nothing automated does it yet.
- **What runs the retention schedule.** `RetentionPolicy` decides what may be
  kept and for how long once somebody supplies the windows (5.5). Nothing yet
  runs on a timer to delete anything, so today a deployment keeps everything.

### 5.7 Generating the two keys

Neither key is generated by the code, on purpose: a key made at start-up would
be a key nobody backed up, and one made by an installer is one everybody has.

```
python -c "import secrets, pathlib; pathlib.Path('secrets/hash.key').write_bytes(secrets.token_bytes(32))"
uv run python -c "from cryptography.hazmat.primitives.asymmetric.ed25519 import Ed25519PrivateKey; from cryptography.hazmat.primitives import serialization; import pathlib; pathlib.Path('secrets/ledger.pem').write_bytes(Ed25519PrivateKey.generate().private_bytes(encoding=serialization.Encoding.PEM, format=serialization.PrivateFormat.PKCS8, encryption_algorithm=serialization.NoEncryption()))"
```

Back both up before starting, and keep them out of the database and out of the
image. Losing the hashing key makes every stored digest unmatchable; losing the
signing key ends the checkpoint chain at the last one published. The public half
of the signing key is what a third party needs, and `/ledger/checkpoint` returns
it with every checkpoint.
...
| Phase 9 | Complete. What it leaves open is deployment policy, not code: authentication, checkpoint publication and a retention schedule — see 5.5 and 5.6 |
